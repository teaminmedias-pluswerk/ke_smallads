...
class tx_kesmallads_pi1 extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
    ...
    public $scriptRelPath = 'pi1/class.tx_kesmallads_pi1.php';	// Path to this script relative to the extension dir.
    public $extKey = 'ke_smallads';								// The extension key.
    public $uploadFolder = 'uploads/tx_kesmallads/'; 			// hmm, maybe I should get the value from $TCA
    public $table = 'tx_kesmallads_smallads';					// Tablenames where the Smallads are stored
    public $mode_selector;										// Mode the plugin is running in. Set via Flexforms in the Backend.
    public $filevar = 'attachment'; 								// Typo3 forgives that name
    public $searchmode = 0; 										// 0=> full list, 1=> short list, only linked headers; set in the plugin
    public $siteRelPath;										// Path to this extension from the main directory.
    public $formName = 'kesmalladsform';						// Name of the HTML-form for new smallads
    ...
    */
    ...
    public function processFormforNewAd() 
    {/*{{{*/
        $content .= '';
        ...
        if (intval($this->conf['ContentMaxChars']) > 0) {
            if (strlen($this->postVars['content']) > intval($this->conf['ContentMaxChars'])) {
                $errors[] = $lcObj->$lcObj->cObjGetSingle('TEXT', $this->conf['ContentTooManyCharsMessage.']);
            }
        }
        if (intval($this->conf['ContentAndTitleMaxChars']) > 0) {
            if (strlen($this->postVars['content']) + strlen($this->postVars['title']) > intval($this->conf['ContentAndTitleMaxChars'])) {
                $errors[] = $lcObj->cObjGetSingle('TEXT', $this->conf['ContentTooManyCharsMessage.']);
            }
        }
        ...
        if ($result) {
            ...
            if (!is_array($updateRecord)) {
                $content .= $lcObj->cObjGetSingle('TEXT', $this->conf['newadCreated.']);
            } else {
                $content .= $lcObj->cObjGetSingle('TEXT', $this->conf['newadUpdated.']);
            }
    ...
    */
    ...
    public function fileTooBig($filesize)
    {/*{{{*/
        return $filesize > $this->conf['maxsize'];
    }/*}}}*/

    public function deleteAd($deleteuid)
    {/*{{{*/
        $content = '';
    ...
    * Template-Setup (look into /Configuration/TypoScript/PluginSetup/setup.txt)
    *
    * if $edituid is set, offer the smallad for editing (only for logged in
    * users)
    */
    public function showForm($edituid = 0)
    {/*{{{*/
        $content = '';
        ...
        if ($edituid) {
            ...
            if ($record['image']) {
                ...
                $content .= $lcObj->cObjGetSingle('IMAGE',  $this->conf['smalladimage.']);
                unset($lcObj);
        ...
        // render the form
        $formContent .= $this->cObj->cObjGetSingle('FORM', $lConf);
...
. 'function checkContentLength() {
    ...
    * @return string
    */
    public function renderJavascriptFunctionsForDynamicForm()
    {/*{{{*/
        $lcObj = GeneralUtility::makeInstance(ContentObjectRenderer::class);
            ...
            foreach ($cat2_configlist as $key => $cat2_config) {
                ...
                foreach ($cat3_configlist as $key => $cat3_config) {
                    ...
                    if ((strstr($key, '.')) && $cat3_config['belongsTo'] == $cat2_config['value']) {
                        ...
                        $content .= '"' . $lcObj->cObjGetSingle('TEXT', $cat3_config) . '|' . $lcObj->cObjGetSingle('TEXT', $cat3_config) . '"';
                        $counter2++;
                    }
                }
...
for (i=0; i<subCategoryList[selectedcat].length; i++) {
    ...
    * @return void
    */
    public function renderInlineJavascriptForDynamicForm()
    {/*{{{*/
        $content = '<script type="text/javascript">' . "\n";
        $content .= '/* <![CDATA[ */' . "\n";
    ...
    */
    public function listViewTeaser()
    {/*{{{*/
        $content='';
        ...
        $content .= $lcObj->cObjGetSingle('TEXT', $this->conf['linktextTeaserShowAll.']);
    ...
    *
    */
    public function listView($edit = 0)
    {/*{{{*/
        // Don't allow editing for non-logged-in users
    ...
    * @return string
    */
    public function dropdownModeSelectorFilter($queryForMode = 'all')
    {/*{{{*/
        $db_whereClause = '';
        $modes = array(
    ...
    * @return string
    */
    public function checkboxModeSelectorFilter()
    {/*{{{*/
        $db_whereClause = '';
    ...
    * @return string
    */
    public function renderDropdownModeSelector()
    {
        $lcObj = GeneralUtility::makeInstance(ContentObjectRenderer::class);
        $content = '';
    ...
    * @return string
    */
    public function renderCheckboxModeSelector()
    {
        $lcObj = GeneralUtility::makeInstance(ContentObjectRenderer::class);
        $content = '';
        ...
        if ($cat2_list) {
            ...
            foreach ($this->conf['smalladForm.']['dataArray.']['12.']['valueArray.'] as $cat_conf) {
                ...
                $cat_label = trim($lcObj->cObjGetSingle('TEXT', $cat_TEXT_obj));
                ...
                */
                ...
                foreach ($this->conf['cat3.'] as $sub_cat_conf) {
                    $content_subcat = '';
                        ...
                        // get label of this category
                    $sub_cat_label = trim($lcObj->cObjGetSingle('TEXT', $sub_cat_conf));
    ...
    */
    public function pi_list_makelist($res, $edit = 0)
    {/*{{{*/
    ...
    * $c : rowcounter
    */
    public function pi_list_row($c, $edit = 0)
    {/*{{{*/
    ...
    {/*{{{*/
        ...
        switch($fN) {
            case "image":
                ...
                return $lcObj->cObjGetSingle('IMAGE', $this->conf['smalladimage.']);
            break;
            case "email":
    ...
    * @return void
    */
    public function getCategoryName($catkey, $valueArray = array())
    {/*{{{*/
        // make a local instance of tslib_cObj
        $lcObj = GeneralUtility::makeInstance(ContentObjectRenderer::class);
        if (is_array($valueArray)) {
            foreach ($valueArray as $cat) {
                ...
                if (strtoupper($cat['value']) == strtoupper($catkey)) {
                    ...
                    return trim($lcObj->cObjGetSingle('TEXT', $this->conf['smalladcontent.cat.']));
                }
            }
        }
    ...
    */
    public function getValueArrayFromLL($lConf, $dataArrayRow, $locallang_index)
    {/*{{{*/
